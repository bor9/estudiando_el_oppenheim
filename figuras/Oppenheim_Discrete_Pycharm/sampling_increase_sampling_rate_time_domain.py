...
# indice relativo de las muestras
for i, nii in enumerate(n2):
    plt.text(na2[i], baseline, '${}$'.format(nii), fontsize=fontsize, ha='center', va='baseline')
...
# xe[n]
x3 = ['nan' if a % M == 0 else 0 for a in n1]
...
plt.plot(nn, xx, '-', lw=1, color=grey)
# señal x[n]
(markers, stemlines, bl) = plt.stem(na1, x3, linefmt='r', markerfmt='sr', use_line_collection=True)
plt.setp(markers, markersize=3.5, zorder=10)
plt.setp(bl, visible=False)
for i, nii in enumerate(n1):
    plt.text(na1[i], baseline, '${}$'.format(nii), fontsize=fontsize, ha='center', va='baseline')
# señal x[nM]
(markers, stemlines, bl) = plt.stem(na2, x2, linefmt='k', markerfmt='sk', use_line_collection=True)
plt.setp(markers, markersize=3.5)
plt.setp(bl, visible=False)
...
plt.text(xmin_ax, ymax_ax, '$\displaystyle x_e[n]=\sum_{k=-\infty}^\infty x[k]\delta[n-kL]$\n$L=3$',
...
plt.plot(nn, xx, '-', lw=1, color=grey)
# señal x[n]
(markers, stemlines, bl) = plt.stem(na1, x1, linefmt='r', markerfmt='sr', use_line_collection=True)
plt.setp(markers, markersize=3.5)
plt.setp(bl, visible=False)
for i, nii in enumerate(n1):
    plt.text(na1[i], baseline, '${}$'.format(nii), fontsize=fontsize, ha='center', va='baseline')
...
plt.text(xmin_ax, ymax_ax, '$x_i[n]$', fontsize=fontsize2, ha='left', va='top')

plt.axis('off')

plt.savefig('sampling_increase_sampling_rate_time_domain.pdf', bbox_inches='tight')
