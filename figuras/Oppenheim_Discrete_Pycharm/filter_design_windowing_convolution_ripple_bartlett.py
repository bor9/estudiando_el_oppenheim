...
w0 = round((nw - 1) / 2)
# espectro de la ventana triangular
#W = np.sin(w * (M + 1) / 2) / np.sin(w / 2)
W = 2 * ((np.sin(w * M / 4) / np.sin(w / 2)) ** 2) / M
W[w0] = M / 2
...
hn[0] = wc / np.pi
hn = np.concatenate((np.flip(hn[1:]), hn)) * signal.windows.bartlett(M + 1)
...
##################################################
# save as pdf image
plt.savefig('filter_design_windowing_convolution_ripple_bartlett.pdf', bbox_inches='tight')
