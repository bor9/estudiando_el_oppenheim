...
# se redefine omega1 y omegaN para que sean
...
# Filtro del filtro pasabajos discreto con w_c=pi/M
omegac = omega1 / (2 * M)
# se redefine que sea
# un número entero de muestras de frecuencia
Nomegac = round(omegac / Domega)
omegac = Nomegac * Domega
H = np.ones((2 * Nomegac + 1,))
H = create_impulse_train_spectrum(Nomega1, H, Nomegac)

# Construcción del espectro de la señal decimada
Nomegas = 2 * NomegaN
Xdec = X[M * NomegaN - Nomegas: M * NomegaN + Nomegas + 1]
Xdec[0] = 0
Xdec = create_impulse_train_spectrum(Nomega1, Xdec, Nomegas)
...
plt.annotate('$\dfrac{\pi}{2}$', xytext=(1.7 * omegaN, 0.5), xycoords='data',
...
plt.annotate("", xytext=(0, 0), xycoords='data', xy=(0, ymax_ax - dymax), textcoords='data',
             arrowprops=dict(width=0.1, headwidth=5, headlength=7, facecolor='black', shrink=0.002))
plt.plot(omega, Xsub * d_heigh_d, 'k-', lw=1.5)
plt.plot(omega, Xsub1 * d_heigh_d, color=grey, lw=1, zorder=1)
plt.plot(omega, Xsub2 * d_heigh_d, color=grey, lw=1, zorder=1)
plt.plot(omega, Xsub3 * d_heigh_d, color=grey, lw=1, zorder=1)
# etiquetas de los ejes
for i in np.arange(-nc, nc + 1):
    omega_i = i * omega1 / 2
    plt.plot([omega_i, omega_i], [0, xtl], 'k-', lw=1)
    ...
    if i == 0:
        ...
        plt.text(omega_i, xtm, '${}{}\pi$'.format(sg, np.absolute(i)),
                 fontsize=fontsize, ha='center', va='baseline')
plt.text(xmax_ax, xtm, '$\omega$', fontsize=fontsize, ha='right', va='baseline')
plt.text(ytm2, d_heigh_d + 0.15, '$\dfrac{1}{MT}=\dfrac{1}{3T}$', fontsize=fontsize, ha='right', va='center')
plt.plot([0, ytl], [d_heigh_d, d_heigh_d], 'k-', lw=1)
...
for i in np.arange(-nc, nc + 1):
    ...
    if i == 0:
        ...
        plt.text(omega_i, xtm, '${}{}\pi$'.format(sg, np.absolute(i)),
                 fontsize=fontsize, ha='center', va='baseline')
plt.text(omegac, xtm, '$\omega_c$', fontsize=fontsize, ha='center', va='baseline')
plt.text(-omegac, xtm, '$-\omega_c$', fontsize=fontsize, ha='center', va='baseline')
plt.text(xmax_ax, xtm, '$\omega$', fontsize=fontsize, ha='right', va='baseline')
plt.text(ytm2, 1, '$1$', fontsize=fontsize, ha='right', va='bottom')
plt.text(ytm, ymax_ax - dymax, '$H_d(e^{j\omega})$', fontsize=fontsize2, ha='left', va='center')
plt.annotate('$\dfrac{\pi}{M}=\dfrac{\pi}{3}$', xytext=(1.2 * omegaN, 0.5), xycoords='data',
             xy=(omegac, 0), textcoords='data', fontsize=fontsize, va="center", ha="left",
             arrowprops=dict(arrowstyle="-|>, head_width=0.1, head_length=0.4", facecolor='black', relpos=(0, 0.5),
                             patchA=None, patchB=None, shrinkA=3, shrinkB=0))
plt.axis('off')
...
plt.annotate("", xytext=(0, 0), xycoords='data', xy=(0, ymax_ax - dymax), textcoords='data',
             arrowprops=dict(width=0.1, headwidth=5, headlength=7, facecolor='black', shrink=0.002))
plt.plot(omega, H * Xs * d_heigh, 'k-', lw=1.5)
# etiquetas de los ejes
for i in np.arange(-nc, nc + 1):
    omega_i = i * omega1 / 2
    plt.plot([omega_i, omega_i], [0, xtl], 'k-', lw=1)
...
plt.text(ytm2, d_heigh, '$\dfrac{1}{T}$', fontsize=fontsize, ha='right', va='center')
plt.plot([0, ytl], [d_heigh, d_heigh], 'k-', lw=1)
plt.text(ytm, ymax_ax - dymax, r'$\tilde{X}(e^{j\omega})=H_d(e^{j\omega})X(e^{j\omega})$',
         fontsize=fontsize2, ha='left', va='center')
plt.annotate('$\dfrac{\pi}{M}=\dfrac{\pi}{3}$', xytext=(1.2 * omegaN, 0.5), xycoords='data',
             xy=(omegac, 0), textcoords='data', fontsize=fontsize, va="center", ha="left",
             arrowprops=dict(arrowstyle="-|>, head_width=0.1, head_length=0.4", facecolor='black', relpos=(0, 0.5),
                             patchA=None, patchB=None, shrinkA=3, shrinkB=0))
...
plt.annotate("", xytext=(0, 0), xycoords='data', xy=(0, ymax_ax - dymax), textcoords='data',
             arrowprops=dict(width=0.1, headwidth=5, headlength=7, facecolor='black', shrink=0.002))
plt.plot(omega, Xdec * d_heigh_d, 'k-', lw=1.5)
# etiquetas de los ejes
for i in np.arange(-nc, nc + 1):
    omega_i = i * omega1 / 2
    plt.plot([omega_i, omega_i], [0, xtl], 'k-', lw=1)
...
plt.text(ytm2, d_heigh_d + 0.15, '$\dfrac{1}{MT}=\dfrac{1}{3T}$', fontsize=fontsize, ha='right', va='center')
plt.plot([0, ytl], [d_heigh_d, d_heigh_d], 'k-', lw=1)
plt.text(ytm, ymax_ax - dymax, r'$\tilde{X}_d(e^{j\omega})$', fontsize=fontsize2, ha='left', va='center')
plt.axis('off')


plt.savefig('sampling_subsampling_freq_domain_decimation.pdf', bbox_inches='tight')
