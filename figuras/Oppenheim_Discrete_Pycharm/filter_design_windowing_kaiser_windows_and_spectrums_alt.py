...
# construcción de las ventanas en tiempo continuo
Nc = 200
nc = np.linspace(0, M, Nc, endpoint=True)
winsc = np.zeros((Nc, Nwins))
alpha = (M - 1) / 2
# espectros
nw = 1024
...
for i in np.arange(Nwins):
    # ventana en tiempo discreto
    wins[:, i] = signal.windows.kaiser(M, betas[i])
    # ventana en tiempo continuo
    winsc[:, i] = i0(betas[i] * np.sqrt(1 - ((nc - alpha) / alpha) ** 2)) / i0(betas[i])
...
xticks = [0, M-1]
xticks_labels = [r'${}$'.format(xtick) for xtick in xticks]
#xticks_labels = ['$0$', '$M$']
yticks = [0, 1]
yticks_labels = [r'${}$'.format(ytick) for ytick in yticks]

leg = [r'$\beta={}$'.format(beta) for beta in betas]

fig = plt.figure(0, figsize=(8, 4), frameon=False)

ax = plt.subplot2grid((Nwins, 11), (0, 0), rowspan=Nwins, colspan=5)
...
for i in np.arange(Nwins):
    ...
    plt.plot(nc, winsc[:, i], ls='-', color=colors_srt[i], lw=1)
...
plt.savefig('filter_design_windowing_kaiser_windows_and_spectrums_beta_alt.pdf', bbox_inches='tight')
...
ymax_ax = 10
ymin_ax = -130
leg = [r'$M={}$'.format(M-1) for M in Ms]

fig = plt.figure(2, figsize=(7, 4), frameon=False)
