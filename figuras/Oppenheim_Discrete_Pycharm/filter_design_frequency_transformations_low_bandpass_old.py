...
# frecuencia w1
dw1 = np. pi / 8
w1 = np.arange(dw1, np.pi - dw, dw1)
w2 = w1 + dw

# k es fijo por ser el ancho de banda fijo
k = np.tan(theta_p / 2) / np.tan(dw / 2)
alphas = -np.cos((w2 + w1) / 2) / np.cos((w2 - w1) / 2)

beta1 = 2 * alphas * k / (k + 1)
beta2 = (k - 1) / (k + 1)

nfrecs = 256
w = np.linspace(0, np.pi, nfrecs)

thetas = -np.arctan2(2 * ((1 + beta2) * np.cos(w[:, np.newaxis]) - beta1) * (1 - beta2) * np.sin(w[:, np.newaxis]),
                     - ((1 + beta2) * np.cos(w[:, np.newaxis]) - beta1) ** 2
                     + ((1 - beta2) * np.sin(w[:, np.newaxis])) ** 2)
...
theta, H = signal.freqz(b, a, worN=Nfrec, whole=False, plot=None, fs=2*np.pi, include_nyquist=False)


# Filtros transformados
Hts = np.empty((Nfrec, len(alphas)))
for i, alpha in enumerate(alphas):
    tnum = [-beta2, beta1[i], -1]
    tden = [1, -beta1[i], beta2]
...
plt.plot([0, theta_p], [w1, w1], '--', lw=1)
plt.plot([theta_p, theta_p], [0, w1[-1]], '--', lw=1, color=grey)
...
plt.plot([0, w1[-1]], [1 - delta_p, 1 - delta_p], '--', lw=1, color=grey)
