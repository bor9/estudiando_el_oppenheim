...
thetas = -np.arctan2(-2 * ks * (np.cos(w[:, np.newaxis]) - alphas) * np.sin(w[:, np.newaxis]),
                     (np.cos(w[:, np.newaxis]) - alphas) ** 2 - (ks * np.sin(w[:, np.newaxis])) ** 2)
# se cambia el intervalo (-pi, pi) a (0, 2pi)
thetas[thetas < 0] += 2 * np.pi

# Expresión de theta equivalente.
#thetas =-2 * np.arctan2(-ks * np.sin(w[:, np.newaxis]), np.cos(w[:, np.newaxis]) - alphas)
...
plt.plot([0, w2[0]], [theta_p2, theta_p2], '--', lw=1, color=grey, zorder=-3)
...
plt.savefig('filter_design_frequency_transformations_low_bandstop_k.pdf', bbox_inches='tight')
