...
thetas = -np.arctan2(2 * k * (np.cos(w[:, np.newaxis]) - alphas) * np.sin(w[:, np.newaxis]),
                     -(k * (np.cos(w[:, np.newaxis]) - alphas)) ** 2 + np.sin(w[:, np.newaxis]) ** 2)

# Expresión de theta equivalente.
#thetas = -np.pi - 2 * np.arctan2(-np.sin(w[:, np.newaxis]), k * (np.cos(w[:, np.newaxis]) - alphas))

gamma1 = 2 * alphas * k / (k + 1)
gamma2 = (k - 1) / (k + 1)
...
plt.plot([1 - delta_p, 1 - delta_p], [theta_sym[1], theta_p], '--', lw=1, color=grey, zorder=-1)
...
plt.gca().set_prop_cycle(None)
plt.plot([w2, w2], [thmin, theta_p], '--', lw=1, zorder=-2)
plt.gca().set_prop_cycle(None)
plt.plot([w2], [theta_p], marker='o', markersize=markersize)
plt.plot([0, w2[-1]], [theta_p, theta_p], '--', lw=1, color=grey, zorder=-3)
...
plt.savefig('filter_design_frequency_transformations_low_bandpass_alpha.pdf', bbox_inches='tight')
