...
idx = omegas <= (wp + ws) / 2
Hd[idx] = omegas[idx] / (2 * np.sin(omegas[idx] / 2))
Hd[0] = 1
...
while 1:
    ...
    E_last = E[new_freq_idx_alternations[j-1]]
    delta_abs = np.abs(delta)
    if abs(E[0]) > abs(E[-1]):
        E1 = E[new_freq_idx_alternations[0]]
        if ((E1 < 0) and E[0] >= delta_abs-tol_alt) or ((E1 > 0) and E[0] <= -delta_abs+tol_alt):
            new_freq_idx_alternations[j] = 0
            j = j + 1
        if j < nalternations:
            new_freq_idx_alternations[j] = nomega
    else:
        if ((E_last < 0) and (E[nomega] >= delta_abs-tol_alt)) or ((E_last > 0) and (E[nomega] <= -delta_abs+tol_alt)):
            new_freq_idx_alternations[j] = nomega
            j = j + 1
        if j < nalternations:
            new_freq_idx_alternations[j] = 0
    ...
    if iterationcount == maxiterationcount - 1:
        break
...
for k in np.arange(iterationcount):
    ...
    plt.plot([omegas[0], omegas[-1]], [delta_list[k], delta_list[k]], 'k--', lw=1)
    plt.plot([omegas[0], omegas[-1]], [-delta_list[k], -delta_list[k]], 'k--', lw=1)
    if k == 0:
...
plt.xlabel('$n$')
plt.savefig('filter_design_fir_optimal_pm_zero_order_hold_Ae_h.pdf', bbox_inches='tight')


wd = np.linspace(0, wp, 100)
Aed = wd / (2 * np.sin(wd / 2))
Aed[0] = 1
...
ymax_ax = 1.08
ymin_ax = 0.98
fig = plt.figure(3, figsize=(8, 3), frameon=False)
# Respuesta en frecuencia
ax = plt.subplot2grid((1, 1), (0, 0), rowspan=1, colspan=1)
plt.ylim(ymin_ax, ymax_ax)
plt.plot(omegas, Ae, 'k-', lw=1.5, zorder=10, label='$A_e(e^{j\omega})$')
plt.plot(wd, Aed, '-', color=grey, lw=1.5, label='$H_d(e^{j\omega})$')
plt.plot(wd, Aed + delta * K, 'k--', lw=1, label='$H_d(e^{j\omega})\pm K\delta$')
plt.plot(wd, Aed - delta * K, 'k--', lw=1)
plt.plot(omegas[new_alt_freq_idx_k], Ae[new_alt_freq_idx_k], **new_alt_marker_style, zorder=15,
         label=r'$\textrm{Alternancias}$')
plt.xlabel(r'$\omega$', fontsize=fs)
plt.xticks(xticks, xticks_labels, usetex=True)
plt.xlim(xmin_ax, xmax_ax)
plt.legend(loc='upper left', fontsize=fs, frameon=False, framealpha=1)
plt.savefig('filter_design_fir_optimal_pm_zero_order_hold_Ae_passband.pdf', bbox_inches='tight')
