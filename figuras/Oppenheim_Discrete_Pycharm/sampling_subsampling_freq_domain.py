...
for i in np.arange(-nc, nc + 1):
    ...
    if i == 0:
        ...
        plt.text(omega_i, xtm2, '${}\dfrac{{\pi}}{{T}}$'.format(sg), fontsize=fontsize, ha='center', va='baseline')
    else:
        plt.text(omega_i, xtm2, '${}\dfrac{{{}\pi}}{{T}}$'.format(sg, np.absolute(i)),
...
plt.text(ytm, ymax_ax - dymax, '$X_s(j\Omega)=X(e^{j\Omega T})$', fontsize=fontsize2, ha='left', va='center')
plt.text(xmin_ax + 4, ymax_ax- 0.2, '$\Omega_s=\dfrac{2\pi}{T}=4\Omega_N$',
         fontsize=fontsize2, ha='left', va='baseline')
plt.axis('off')
...
plt.text(ytm2, d_heigh, '$\dfrac{1}{T}$', fontsize=fontsize, ha='right', va='center')
plt.plot([0, ytl], [d_heigh, d_heigh], 'k-', lw=1)
plt.text(ytm, ymax_ax - dymax, '$X(e^{j\omega/2})$', fontsize=fontsize2, ha='left', va='center')
plt.text(xmin_ax + 4, ymax_ax - 0.2, '$M=2$', fontsize=fontsize2, ha='left', va='baseline')
plt.axis('off')
...
plt.text(ytm2, d_heigh, '$\dfrac{1}{T}$', fontsize=fontsize, ha='right', va='center')
plt.plot([0, ytl], [d_heigh, d_heigh], 'k-', lw=1)
plt.text(ytm, ymax_ax - dymax, '$X(e^{j(\omega-2\pi)/2})$', fontsize=fontsize2, ha='left', va='center')
plt.axis('off')
...
plt.annotate("", xytext=(0, 0), xycoords='data', xy=(0, ymax_ax - dymax), textcoords='data',
             arrowprops=dict(width=0.1, headwidth=5, headlength=7, facecolor='black', shrink=0.002))
plt.plot(omega, Xsub * d_heigh / M, 'k-', lw=1.5)
# etiquetas de los ejes
for i in np.arange(-nc, nc + 1):
    omega_i = i * omega_a / 2
    plt.plot([omega_i, omega_i], [0, xtl], 'k-', lw=1)
    ...
    if i == 0:
        ...
        plt.text(omega_i, xtm, '${}{}\pi$'.format(sg, np.absolute(i)), fontsize=fontsize, ha='center', va='baseline')
plt.text(xmax_ax, xtm, '$\omega$', fontsize=fontsize, ha='right', va='baseline')
plt.text(ytm2, d_heigh / M + 0.1, '$\dfrac{1}{MT}=\dfrac{1}{2T}$', fontsize=fontsize, ha='right', va='center')
plt.plot([0, ytl], [d_heigh / M, d_heigh / M], 'k-', lw=1)
plt.text(ytm, ymax_ax - dymax, '$X_d(e^{j\omega})=\dfrac{1}{2}[X(e^{j\omega/2})+X(e^{j(\omega-2\pi)/2})]$',
         fontsize=fontsize2, ha='left', va='center')
plt.axis('off')

plt.savefig('sampling_subsampling_freq_domain.pdf', bbox_inches='tight')
